...
from app.views.games_views import CrystallizationView
...
class CrystallizationService:
    ...
    async def start_game(interaction: ApplicationCommandInteraction, bet: int):
        ...
        embed = await games_embeds.format_crystallize_embed(bet=bet)
        view = CrystallizationView(
            bet=bet,
    ...
    async def continue_game(self, interaction: MessageInteraction):
        ...
        if random.random() < (state.loss_chance / 100.0):
            loss_embed = await games_embeds.format_crystallize_loss_embed(state.bet)
        ...
        new_loss_chance = state.loss_chance + (chance_increment * 100)
        new_multiplier = round(state.multiplier + multiplier_increment, 2)
        new_potential_win = int(state.bet * new_multiplier)

        embed = await games_embeds.format_crystallize_embed(bet=state.bet)
        view = CrystallizationView(
            bet=state.bet,
    ...
    async def cash_out(self, interaction: MessageInteraction):
        ...
        state = self._parse_state_from_components(interaction.message.components)
        win_embed = await games_embeds.format_crystallize_win_embed(
